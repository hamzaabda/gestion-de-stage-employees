...
exports.calculerNombreUtilisateursParRole = async (req, res, next) =>{

    try {
        const countAdmins = await User.countDocuments({ role: 'admin' });
        const countEtudiants = await User.countDocuments({ role: 'etudiant' });
        const countRecruteurs = await User.countDocuments({ role: 'recruteur' });
    
        const resultat = {
          admins: countAdmins,
          etudiants: countEtudiants,
          recruteurs: countRecruteurs
        };
    
        res.status(200).json(resultat);
      } catch (error) {
        res.status(500).json({ error: 'Erreur lors du calcul du nombre d\'utilisateurs par rôle' });
      }
    }
